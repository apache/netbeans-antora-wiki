...
|`java.lang.Object` (yes, you can have a property of Object and there is an editor for it - the user can select from all objects of a type in the xref:./DevFaqDefaultLookup.adoc[default Lookup] or a specific lookup [see below] using a combo box) |`superClass` |`java.lang.Class` |The superclass, passed to `Lookup.getDefault().lookupAll()` to find all possible values 
