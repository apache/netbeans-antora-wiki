...
image:wiki/notification_FitnessViaPostMortem.png[]
...
* image:wiki/yes.png[] Have a meeting with profiler guys
* image:wiki/yes.png[] 'Jarda': Write the plan
* image:wiki/yes.png[] *native wait* - treat Object.wait and _link:[native.html[[native]_ state equally (e.g. zero execution time)
* image:wiki/yes.png[] *two times* - output two times (execution time, wall clock time) into the .nps file
* image:wiki/yes.png[] *display two times* - make sure the UI represents the two times in some columns
* image:wiki/yes.png[] *no sum for threads* - the sum of all threads execution is incorrect (in sampling or on multicore CPUs), thus hide it
* image:wiki//yes.png[] *JMX* - rewrite the sampler to JMX. This will open the door to possible use in VisualVM as well as provide more info about the real thread execution time
* image:wiki//yes.png[] *overhead* is slightly less than 10% - _Jarda_: measure overhead of sampling (I ran the sieve of erastothenes for first 100000 numbers. Without self profiling 4.1s+-100ms with profiling 4.3ms+-100ms)
* image:wiki//yes.png[] *inaccuracy*: compute average snapshot time and ignore intervals that are far (more than 30ms) from average. Potentially don't report these snapshots at all as a slowdown.
...
* image:wiki/yes.png[] *format*: change the sampling format to be (more or less) GZip of stacktraces. Processing to tree structure would occur later. Done for NetBeans 6.9
