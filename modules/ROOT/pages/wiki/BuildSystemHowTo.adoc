...
NOTE: all API classes and interfaces are referred to by simple name. You can look up the Javadoc for any of these classes quickly using the link:{apidoclink}allclasses.html[master class index] for the NetBeans APIs.
...
Custom in-VM tasks:: You can also define your own in-VM tasks and use them the same way as the predefined ones. See the link:{apidoclink}org-apache-tools-ant-module/org/apache/tools/ant/module/spi/package-summary.html#register-defs[Ant SPI] for details.
