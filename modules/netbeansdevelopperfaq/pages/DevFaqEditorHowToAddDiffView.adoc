...
:page-position: 19
:page-aliases: ROOT:wiki/DevFaqEditorHowToAddDiffView.adoc
...
* xref:front::blogs/geertjan/getting_started_with_diff_viewer.adoc[Getting Started with Diff Viewer from Scratch]
* xref:front::blogs/geertjan/netbeans_diff_api.adoc[NetBeans Diff API]
* link:{apidoclink}org-netbeans-modules-diff/overview-summary.html[{apidoclink}org-netbeans-modules-diff/overview-summary.html]

== Show editable diff for nodes with FileObject in node's lookup

* xref:front::blogs/geertjan/how_to_create_an_editable.adoc[How to Create an Editable Diff Viewer]
