...
Our editor component will be fairly simple.  It will have two constructors, one which takes a `link:{apidoclink}org-openide-loaders/org/openide/loaders/DataObject.html[DataObject]` (the file) and one which has no arguments:
...
* We will implement our own subclass of `link:{apidoclink}org-openide-nodes/org/openide/cookies/OpenCookie.html[OpenCookie]`, which can create and open an instance of our editor, and remember and reuse that editor on subsequent invocations
...
* Make a `link:{apidoclink}org-openide-nodes/org/openide/cookies/SaveCookie.html[SaveCookie]` available, which is what the various built-in Save actions operate on
