...
:page-position: 1
:page-aliases: ROOT:wiki/DevFaqLookup.adoc

include::front::partial$database.adoc[]

link:{apidoclink}org-openide-util-lookup/org/openide/util/Lookup.html[Lookup] is a mechanism for finding instances of objects.  It is pervasively used in NetBeans APIs.  The general pattern is that you pass a Class object and get back an instance of that class or null.  See the Javadoc for links to articles describing its inspiration and purpose.
...
The primary purpose of Lookup is decoupling - it makes it possible to use generic objects to get very specific information, without having to cast objects to a specific type.  Confused yet?  It's simple.  Take the example of link:{apidoclink}org-openide-awt/org/netbeans/api/actions/Openable.html[Openable] - it has one method, `open()` that will open a file in the editor.
...
The fact is that it is the Image module that makes it possible to open `.png` files in xref:front::index.adoc[NetBeans].  But the POV-Ray tutorial does not need to know or care that the Image module exists, or what it does - it simply says "open this".

The common pattern you'll see for Lookup usage is one where there are three components:

* Module A is pure xref:wiki::wiki/API_Design.adoc[API] - it provides some interfaces
...
Some other module entirely actually registers an implementation of this interface in the xref:./DevFaqLookupDefault.adoc[default Lookup].  link:{apidoclink}org-openide-awt/org/openide/awt/StatusDisplayer.html[StatusDisplayer] is a good example of this pattern.
...
If you need to merge together more than one lookup (for example, the lookup provided from `Node.getCookieSet().getLookup()` and one of your own which you will add and remove objects from), you can simply use link:{apidoclink}org-openide-util-lookup/org/openide/util/lookup/ProxyLookup.html[ProxyLookup] - i.e. `new ProxyLookup (lookup1, lookup2, Lookups.singleton(someObject), ...)`
